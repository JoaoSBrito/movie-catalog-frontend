...
export const Card = styled.div`
  ...
  @media (max-width: ${(props) =>
      props.theme?.breakpoints?.mobile || "480px"}) {
    height: 340px;
...
export const PosterPlaceholder = styled.div`
  ...
  background: linear-gradient(
    135deg,
    ${(props) => props.theme?.colors?.surface || "#1a365d"} 0%,
    ${(props) => props.theme?.colors?.card || "#2d3748"} 100%
  );
  display: flex;
  align-items: center;
...
export const FavoriteButton = styled.button<{ isFavorite: boolean }>`
  ...
  color: ${(props) =>
    props.isFavorite ? props.theme?.colors?.accent || "#ffd700" : "white"};
  border: none;
  display: flex;
  align-items: center;
  justify-content: center;
