...
export const MovieHero = styled.div`
  position: relative;
  height: 300px;
  background: linear-gradient(to bottom, rgba(0, 0, 0, 0.1), ${(props) => props.theme?.colors?.surface || "#1a365d"});
  overflow: hidden;
...
export const BackdropImage = styled.div<{ backdropUrl?: string }>`
  ...
  right: 0;
  bottom: 0;
  background-image: ${(props) => (props.backdropUrl ? `url(${props.backdropUrl})` : "none")};
...
export const PosterPlaceholder = styled.div`
  width: 100%;
  aspect-ratio: 2/3;
  background: linear-gradient(135deg, ${(props) => props.theme?.colors?.card || "#2d3748"} 0%, ${(props) => props.theme?.colors?.background || "#0d253f"} 100%);
  border-radius: ${(props) => props.theme?.borderRadius?.lg || "12px"};
  display: flex;
  align-items: center;
...
export const FavoriteButton = styled.button<{ isFavorite: boolean }>`
  ...
  background: ${(props) => (props.isFavorite ? props.theme?.colors?.accent || "#ffd700" : "rgba(255, 255, 255, 0.1)")};
  color: ${(props) => (props.isFavorite ? "#000" : props.theme?.colors?.text || "#ffffff")};
  border: none;
  ...
  &:hover {
    background: ${(props) => (props.isFavorite ? "#e6c200" : "rgba(255, 255, 255, 0.2)")};
